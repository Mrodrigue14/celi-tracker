...
class OvercontributionTest {
    ...
    private val limits = listOf(
        ...
        AnnualLimit(Account.TFSA, 2020, money("6000.00")),
    )

    private fun excesses(transactions: List<Transaction>, upTo: YearMonth) = Overcontribution.tfsaExcesses(TfsaEngine.roomByYear(profile, limits, transactions, upTo.year), transactions, upTo)

    private fun tx(date: String, type: TransactionType, amount: String) = Transaction(Account.TFSA, LocalDate.parse(date), type, money(amount))

    @Test
    fun `no excess when contributions stay within room`() {
        val transactions = listOf(tx("2019-03-15", TransactionType.DEPOSIT, "6000.00"))

        val excesses = excesses(transactions, upTo = YearMonth.of(2019, 12))
    ...
    fun `an excess persists every month until the end of the year`() {
        ...
        assertEquals(money("4000.00"), excesses.first().maxExcess)
        assertEquals(money("40.00"), excesses.first().penalty)
        assertEquals(12, excesses.last().month)
        assertEquals(money("4000.00"), excesses.last().maxExcess)
    }
    ...
    fun `re-contributing an amount withdrawn the same year recreates the excess`() {
        ...
        val excesses = excesses(transactions, upTo = YearMonth.of(2019, 12)).associateBy { it.month }

        assertTrue(excesses[2] == null)
        assertTrue(excesses[5] == null)
        assertEquals(money("6000.00"), excesses.getValue(6).maxExcess)
        assertEquals(money("60.00"), excesses.getValue(6).penalty)
        // June through December: 7 months.
        assertEquals(7, excesses.size)
        assertEquals(money("6000.00"), excesses.getValue(12).maxExcess)
    }
    ...
    fun `a withdrawal cancels the excess but the month remains billed`() {
        ...
        assertEquals(money("1000.00"), excesses.getValue(3).maxExcess)
        assertEquals(money("10.00"), excesses.getValue(3).penalty)
        // April is billed on the month's highest excess: 1000 until the 15th.
        assertEquals(money("1000.00"), excesses.getValue(4).maxExcess)
        assertTrue(excesses[5] == null)
    }
    ...
    fun `the excess is absorbed by the following year's room`() {
        ...
        val excesses = excesses(transactions, upTo = YearMonth.of(2020, 12))
    ...
    fun `no excess without a transaction`() {
        val excesses = excesses(emptyList(), upTo = YearMonth.of(2019, 12))
